...
def get_all_rooms(request):
	...
	# url = "http://192.168.1.32:5000/rooms"
	...
	# url = f"http://192.168.1.32:5000/all_ips"
...
def get_room(request,room_id):
	...
	# url = f"http://192.168.1.32:5000/room/{room_id}"
	...
	# logs_url = f"http://192.168.1.32:5000/room/{room_id}/ips"
	...
	# print(logs)
	log_dict={}
	if logs['climate_schedule_log']:
		for key,log in enumerate(logs['climate_schedule_log']):
			try:
				log_name = log[key]['name']
			except Exception as e:
				log_name = log[0]['name']
				pass
			print(log_name)
			log_dict.update({log_name:log})
...
def put_room(request):
	if request.method == 'POST':
		...
		# url = f'http://192.168.1.32:5000/rooms'
...
def delete_ip(request,room_id,ip_id):
	...
	# url = f'http://192.168.1.32:5000/room/{room_id}/ip/{ip_id}'
...
def put_schedule(request, room_id):
	if request.method == 'POST':
		...
		if 'interval_hour' in request.POST:
			...
			# url = f'http://192.168.1.32:5000/relayinterval'
			...
			# url = f'http://192.168.1.32:5000/room/{room_id}/relayinterval/{relayinterval_id}'
			...
			# url = f'http://192.168.1.32:5000/relayschedule'
			...
			# url = f'http://192.168.1.32:5000/room/{room_id}/relayschedule/{relayschedule_id}'
...
def patch_schedule(request, room_id, climate_schedule_id):
	if request.method == 'POST':
		...
		# url = f"http://192.168.1.32:5000/room/{room_id}/relayschedule/{climate_schedule_id}"
...
def patch_interval(request, room_id, climate_interval_id):
	if request.method == 'POST':
		...
		# url = f"http://192.168.1.32:5000/room/{room_id}/relayinterval/{climate_interval_id}"
...
def put_climate(request, room_id):
	if request.method == 'POST':
		...
		# url = f'http://192.168.1.32:5000/room/{room_id}/climate'
		...
		# url = f'http://192.168.1.32:5000/room/{room_id}/climate/{climate_id}'
		# url = f'http://192.168.1.37:5000/room/{room_id}/climate/{climate_id}'
		# url = f'http://10.42.0.1:5000/room/{room_id}/climate/{climate_id}'
		payload = {
			'name':request.POST['name'],
			'buffer_parameters':request.POST['buffer_parameters'],
			'co2_parameters':request.POST['co2_parameters'],
			'co2_buffer_parameters':request.POST['co2_buffer_parameters'],
			...
			'climate_start_time':request.POST['climate_start_time'],
			'climate_end_time':request.POST['climate_end_time'],
		}
		response = requests.put(url, data=payload)
		print(response.json())
		return redirect(f'/rooms/get_room/{room_id}')
...
def relay_control(request, room_id):
	if request.method == 'POST':
		...
		# url = f"http://192.168.1.32:5000//relay_control"
